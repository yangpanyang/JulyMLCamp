...
def float_list_to_int_dict(lst, bucket_size):
    ...
    # 遍历一遍数据列，分别将值分桶
    for v in lst:
        # 将 val 映射到对应的 hash 桶内
        k = int( round(v / bucket_size, 0) )
...
def pformat_histogram(d, bucket_size, char='+', width=80, key_format='%6.3f'):
    ...
    Returns a string."""
    # 解题思路：（1）输出的字符长度width必须要满足要求
    # 字符包括：dict显示 + char字符
    
    # 获取dict内key值转为string后的最大占位列数
    key_width = max(len(key_format % (k * bucket_size)) for k in d.keys())
    # 获取dict内value值转为string后的最大占位列数 ==> 找到最大数
    max_value = max( d.values() )
    value_width = len( str( max_value ) )
    # 其余字符占位数
    other_width = len( ' []:' )
    # 计算直方图可表示的最多字符数
    width -= ( key_width + value_width + other_width )
    # 若表示位置不够，则抛出异常
    if width <= 0:
    ...
    for k in sorted( d.keys() ):
        # 归一化打印字符的长度
        bar_len = int( round(float(d[k]) * width / max_value) )
        # 用char实现可视化
        bar = char * bar_len

        # 加入打印list
        # python3的格式语法推荐以“{}”为占位符:>表示右对齐，<表示左对齐
        print_list.append("{:>{}} [{:>{}}]:{}".format(key_format % (k * bucket_size), key_width,
                                          d[k], value_width,
                                          bar))
        '''#python2.7写法:-表示左对齐并用空格补位，*可自定义设置打印的最大长度
        print_list.append('%-*s [%-*d]:%s' % (key_width, key_format % (k * bucket_size),
                                              value_width, d[k],
                                              bar) )
        '''
